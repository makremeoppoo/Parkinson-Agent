...
function PatientAnalyses({
  ...
  useEffect(() => {
    ...
    getToken()
      .then((tok) =>
        fetch(
          `${CONFIG.serverUrl}/patients/${encodeURIComponent(patient.code)}/analyses`,
          {
  ...
  if (loading)
    return (
      <p style={{ color: "#64748b", fontSize: ".8rem", padding: "0.5rem 0" }}>
        Loading…
      </p>
    );
  if (!records.length)
    return (
      <p style={{ color: "#475569", fontSize: ".8rem", padding: "0.5rem 0" }}>
        No analyses yet.
      </p>
    );
...
function PatientCard({
  ...
  const handleDownloadPdf = async () => {
    const tok = await getToken();
    window.open(
      `${CONFIG.serverUrl}/results/report?patientCode=${encodeURIComponent(patient.code)}&token=${encodeURIComponent(tok)}&format=print`,
      "_blank",
    );
  };
    ...
    <div
      ...
      <div
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <div>
            ...
            <p style={{ color: "#64748b", fontSize: ".72rem", margin: 0 }}>
              Doctor Dashboard
            </p>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <p
          ...
          Add a patient code to generate their unique analysis link, then share
          it.
        </p>
        ...
        <form
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700,
              fontSize: ".9rem",
              cursor: adding || !newCode.trim() ? "not-allowed" : "pointer",
              whiteSpace: "nowrap",
            }}>
            <Plus size={16} />
        ...
        {loading ? (
          ...
          <div
            ...
            <p style={{ fontSize: ".85rem" }}>
              Add a patient code above to get started.
            </p>
          </div>
